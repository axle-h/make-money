...
type PartialParsedStatement = Partial<Omit<ParsedStatement, 'transactions'>> &
  Pick<ParsedStatement, 'transactions'>
...
export async function parseStatementFile(file: File): Promise<ParsedStatement> {
  ...
  const transactions = partial.transactions.sort((a, b) =>
    ...
    await Promise.all(
      qif.transactions.map(async (t) => {
        ...
        if (!date || amount === undefined || !payee) {
          throw new Error(
            `cannot parse qif transaction ${JSON.stringify(t, undefined, 2)}`
          )
        }
        ...
        let name = payee
          .replace(
            /\s+ON\s+\d{2}[-\/]\d{2}[-\/]\d{4}[,\s]+-?\d+(?:\.\d+)?(?:GBP)?\s*$/i,
            ''
          )
          .replace(/&amp;/, '&')
          .trim()
        ...
        const xferPaymentMatch = name.match(
          /^BILL\s+PAYMENT\s+VIA\s+FASTER\s+PAYMENT\s+TO\s+([^,]+)\s+REFERENCE\s+([^,]+)[,\s]+MANDATE/i
        )
        const xferReceiptMatch = name.match(
          /FASTER\s+PAYMENTS\s+RECEIPT\s+REF\.(.+)\s+FROM\s+([^,]+)/i
        )
        const ddMatch = name.match(
          /DIRECT\s+DEBIT\s+PAYMENT\s+TO\s+(.+)\s+REF\s+([^,]+)[\s,]+MANDATE\s+NO\s+([^,]+)/i
        )
        const cashMatch = name.match(
          /CASH\s+WITHDRAWAL\s+AT\s+(.+)\s+ATM\s+([^,]+)[,\s]+([^,]+)/i
        )
        const giroCreditMatch = name.match(
          /BANK\s+GIRO\s+CREDIT\s+REF\s+([^,]+), ([^,]+)/i
        )
        ...
        return {
          ...
          externalId: await sha256(
            `${partial.date.toISOString()}:${partial.amount}:${partial.type}:${partial.name}:${partial.description}`
          ),
        }
      })
    )
