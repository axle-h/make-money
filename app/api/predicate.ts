...
function parsePredicate(predicate: string): PNode {
  ...
  function parseExpressionValue(): ExpressionValue {
    ...
    if (stringLiteral) {
      return {
        type: "LITERAL",
        value: stringLiteral[1].replaceAll("\\'", "'"),
      };
    }
...
function evaluateSyntaxTree(node: PNode, obj: Record<string, Leaf>): boolean {
  function extractValue(ref: ExpressionValue): Leaf {
    switch (ref.type) {
      ...
      default:
        throw new Error(
          `Unsupported node type: ${(ref as ExpressionValue).type}`,
        );
    }
  }
  ...
  switch (node.type) {
      ...
      switch (operator) {
        ...
        case "LIKE": {
          if (typeof left !== "string" || typeof right !== "string") {
            throw new Error(
              "both arguments of LIKE expression must be a string",
            );
          }
          const regex = new RegExp(
            escapeRegExp(right).replaceAll("%", ".*"),
            "i",
          );
    ...
    case "LOGICAL": {
      ...
      switch (operator) {
        case "AND":
          return (
            evaluateSyntaxTree(left, obj) && evaluateSyntaxTree(right, obj)
          );
        case "OR":
          return (
            evaluateSyntaxTree(left, obj) || evaluateSyntaxTree(right, obj)
  ...
  switch (node.type) {
    case "CONDITION": {
      ...
      if (!parameter || !literal) {
        throw new Error(
          `expected one parameter and one literal but got ${left.type} and ${right.type}`,
        );
      }
    ...
    case "LOGICAL": {
      ...
      switch (operator) {
        case "AND":
          return {
            AND: [toPrismaWhereCondition(left), toPrismaWhereCondition(right)],
          };
        case "OR":
          return {
            OR: [toPrismaWhereCondition(left), toPrismaWhereCondition(right)],
          };
        default:
          throw new Error(`Unsupported operator: ${operator}`);
      }
