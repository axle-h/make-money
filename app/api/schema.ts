...
export class Schema {
  ...
  static readonly StatementQuery = this.PaginatedQuery.extend({
    ...
    orderBy: z
      .enum(['dateUploaded', 'startDate', 'endDate', 'accountId'])
      .optional(),
  })

  private static readonly StringFilter = z
    .object({
  ...
  static readonly UpdateTransactionRequest = z.object({
    ...
    categories: z
      ...
      .refine(
        (cats) =>
          cats.length === 0 ||
          cats.map((cat) => cat.fraction).reduce((x, y) => x + y) === 1,
  ...
  static readonly NewCategoryRule = z.object({
    ...
    predicate: z
      .string()
      .refine(
        validateTransactionPredicate,
        'Must be a valid transaction predicate'
      ),
...
export function accountNumber(
  account: Pick<Account, 'accountType' | 'accountNumber' | 'sortCode'>
) {
...
export function accountName(
  account: Pick<Account, 'accountNumber' | 'bankName'>
) {
...
export type PaginatedTransactionQuery = z.infer<
  (typeof Schema)['PaginatedTransactionQuery']
>
export type TransactionQuery = z.infer<(typeof Schema)['TransactionQuery']>
export type CategorizedTransactionQuery = z.infer<
  (typeof Schema)['CategorizedTransactionQuery']
>
...
export type UpdateTransactionRequest = z.infer<
  (typeof Schema)['UpdateTransactionRequest']
>
