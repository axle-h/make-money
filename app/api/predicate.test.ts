...
describe('predicate', () => {
  ...
  it('filters transactions', () => {
    const predicate = new Predicate(
      "externalId == 'some-external-id' AND type == 'some-type' AND name == 'some-name' AND description == 'some-description' AND amount > 2.49"
    )
    expect(predicate.evaluate(EXAMPLE_TRANSACTION)).toBe(true)
    expect(
      predicate.evaluate({ ...EXAMPLE_TRANSACTION, name: 'some-other-name' })
    ).toBe(false)
  })
  ...
  it('handles like contains', () => {
    expect(
      new Predicate("name LIKE '%asos%'").evaluate({ name: 'Klarna*asos.com' })
    ).toBe(true)
    expect(
      new Predicate("name LIKE '%MYDENTIST%'").evaluate({
        name: '467 MYDENTIST CHESTERF',
      })
    ).toBe(true)
  })
})
