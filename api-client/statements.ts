...
export type ListStatementsRequest = ApiRequest<'list-statements'> &
  StatementQuery

export class StatementApi {
  async list(query: StatementQuery): Promise<Paginated<Statement>> {
    const response = await fetch(
      '/api/statements?' + stringifySearchParams(query)
    )
...
export function useStatements(query: StatementQuery) {
  ...
  const { data: statements, ...rest } = useSWR(key, () =>
    statementApi.list(query)
  )
