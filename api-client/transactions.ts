...
export class TransactionApi {
  async list(
    ...
    const response = await fetch(
      '/api/transactions?' + stringifySearchParams(query)
    )
  ...
  async listCategorized(
    ...
    const response = await fetch(
      '/api/transactions/categorized?' + stringifySearchParams(query)
    )
  ...
  async meta(query: TransactionQuery): Promise<TransactionMeta> {
    const response = await fetch(
      '/api/transactions/meta?' + stringifySearchParams(query)
    )
...
export function useTransactions(query: PaginatedTransactionQuery) {
  ...
  const { data: transactions, ...rest } = useSWR(key, () =>
...
export function useCategorizedTransactions(query: CategorizedTransactionQuery) {
  ...
  const { data: transactions, ...rest } = useSWR(key, () =>
    transactionApi.listCategorized(query)
  )
  return { transactions, ...rest }
}
...
export function useTransactionMeta(query: TransactionQuery) {
  ...
  const { data: transactionMeta, ...rest } = useSWR(key, () =>
    transactionApi.meta(query)
  )
...
export async function mutateTransactions() {
  await mutate((key) =>
    isApiKey(key, 'list-transactions', 'get-transaction-meta')
  )
}
