...
type PartialCategorizedTransaction = Partial<
  Stringified<CategorizedTransaction>
>

function transaction(
  partial: PartialCategorizedTransaction
...
function transactions(
  partials: PartialCategorizedTransaction[]
): CategorizedTransaction[] {
...
describe('data', () => {
  describe('aggregate by category', () => {
    ...
    it('aggregates to multiple, orders largest to smallest', () => {
      ...
      expect(aggregateByCategory([...expense1s, ...expense2s])).toEqual(
        expected
      )
    })
    ...
    it('collapses to other', () => {
      ...
      const observed = aggregateByCategory(
        transactions([
  ...
  describe('time series', () => {
    it('aggregates to day', () => {
      const observed = timeSeries(
        ...
        parseIsoUtcDatetime('2024-06-09'),
        parseIsoUtcDatetime('2024-06-10')
      )
    ...
    it('fills in time range with 0s', () => {
      const observed = timeSeries(
        transactions([{ date: '2024-06-09', debit: -19.45 }]),
        parseIsoUtcDatetime('2024-06-09'),
        parseIsoUtcDatetime('2024-06-10')
      )
    ...
    function timeSeriesShouldBe(
      observed: TimeSeriesEntry[],
      expected: Stringified<TimeSeriesEntry>[]
    ) {
      expect(
        observed.map(({ date, ...t }) => ({
          ...t,
