...
function getTaxYearOfDate(date: Date): number {
  return isAfter(
    date,
    new UTCDate(UTCDate.UTC(date.getUTCFullYear(), 3, 6, 0, 0, 0, 0))
  )
    ? date.getUTCFullYear()
...
export function formatDateRange(start: Date, end: Date): string {
  ...
  const dates =
    yearFrom === currentYear && yearTo === currentYear
...
export function getRangeMonthsToNow(
  months: number,
  now: Date = utcNow()
): DateRange {
